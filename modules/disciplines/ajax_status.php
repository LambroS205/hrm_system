<?php
// modules/disciplines/ajax_status.php - API cập nhật trạng thái Kỷ luật qua Kéo - Thả Kanban
...
if (!has_permission('disciplines', 'approve')) {
    http_response_code(403);
    echo json_encode(['success' => false, 'message' => 'Bạn không có quyền phê duyệt hoặc thực thi quyết định kỷ luật.']);
    exit;
}
...
$valid_statuses = ['pending', 'approved', 'executed', 'cancelled'];
...
try {
    ...
    if (!$discipline) {
        ...
        echo json_encode(['success' => false, 'message' => 'Không tìm thấy biên bản kỷ luật này.']);
        exit;
    }

    $updateSql = "UPDATE disciplines SET status = :status";
    ...
    echo json_encode([
        'success' => true,
        'message' => "Đã chuyển [{$discipline['discipline_code']}] sang trạng thái \"{$statusLabels[$new_status]}\".",
